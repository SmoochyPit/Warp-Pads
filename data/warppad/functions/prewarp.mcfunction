...
#Tag player if they're a "passenger" (on the warp pad but not using it)
	...
	execute as @e[tag=wp.pad] at @s as @e[distance=..2,tag=!wp.using,tag=!wp.pad,tag=!wp.locIcon,tag=!wp.locTemp,type=!#warppad:nonwarpable] positioned ~-2 ~-.5 ~-2 run tag @s[dx=3,dy=0,dz=3] add wp.passenger
