...
#warp or retry - note: "wp.hasSelect" is a requirement to run, and may break in complicated ways without.
	...
	execute if score @e[tag=wp.selCastIcon,limit=1,sort=nearest] wp.tempID = @e[tag=wp.pad,limit=1,sort=nearest,tag=wp.hasSelect] wp.tempID at @e[tag=wp.pad,limit=1,sort=nearest,tag=wp.hasSelect] run function warppad:warpplayers
	execute if entity @e[tag=wp.pad,tag=!wp.wlIterated,limit=1,sort=nearest,tag=wp.hasSelect] at @e[tag=wp.pad,tag=!wp.wlIterated,limit=1,sort=nearest,tag=wp.hasSelect] run function warppad:warplaunch